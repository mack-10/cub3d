...
int		ray_c(t_val *lst)
{
	for (int y = 0; y < screenH; y++)
	{
		float rayDirX0 = lst->dirX - lst->planeX;
		float rayDirY0 = lst->dirY - lst->planeY;
		float rayDirX1 = lst->dirX + lst->planeX;
		float rayDirY1 = lst->dirY + lst->planeY;
		int p = y - screenH / 2;
		float posZ = 0.5 * screenH;
		float rowDistance = posZ / p;
		float floorStepX = rowDistance * (rayDirX1 - rayDirX0) / screenW;
		float floorStepY = rowDistance * (rayDirY1 - rayDirY0) / screenW;
		float floorX = lst->posX + rowDistance * rayDirX0;
		float floorY = lst->posY + rowDistance * rayDirY0;

		for (int x = 0; x < screenW; ++x)
		{
			int cellX = (int)(floorX);
			int cellY = (int)(floorY);
			int tx = (int)(textureW * (floorX - cellX)) & (textureW - 1);
			int ty = (int)(textureH * (floorY - cellY)) & (textureH - 1);
