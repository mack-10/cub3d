...
static void		step(t_main *lst)
{
	...
	{
		lst->ray.step_x = 1;
		lst->ray.sidedist_x = (lst->ray.map_x + 1.0 - lst->pos_x) *
			lst->ray.deltadist_x;
	}
	...
	{
		lst->ray.step_y = 1;
		lst->ray.sidedist_y = (lst->ray.map_y + 1.0 - lst->pos_y) *
			lst->ray.deltadist_y;
	}
}
...
{
	if (lst->ray.side == 0)
		lst->ray.perpwalldist = (lst->ray.map_x - lst->pos_x +
			(1 - lst->ray.step_x) / 2) / lst->ray.raydir_x;
	else
		lst->ray.perpwalldist = (lst->ray.map_y - lst->pos_y +
